...
namespace OpenPOS.CCO115Interop
{
    ...
    [ServiceObject(DeviceType.Scanner, "OpenPOS 1.15 Scanner", "OPOS Scanner CCO Interop", 1, 15)]
    public class OpenPOSScanner : Scanner, ILegacyControlObject, IDisposable
    {
        private OpenPOS.Devices.OPOSScanner _cco = null;
        private const string _oposDeviceClass = "Scanner";
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (!_disposedValue)
            {
                ...
                if (_cco != null)
                {
                    _cco.DataEvent -= (_IOPOSScannerEvents_DataEventEventHandler)_cco_DataEvent;
                    _cco.DirectIOEvent -= (_IOPOSScannerEvents_DirectIOEventEventHandler)_cco_DirectIOEvent;
                    _cco.ErrorEvent -= (_IOPOSScannerEvents_ErrorEventEventHandler)_cco_ErrorEvent;
                    _cco.StatusUpdateEvent -= (_IOPOSScannerEvents_StatusUpdateEventEventHandler)_cco_StatusUpdateEvent;
                    _cco = null;
                }
        ...
        public override void Open()
        {
            ...
            if (_cco == null)
            {
                try
                {
                    ...
                    _cco = new OpenPOS.Devices.OPOSScanner();

                    // Register event handler
                    _cco.DataEvent += new _IOPOSScannerEvents_DataEventEventHandler(_cco_DataEvent);
                    _cco.DirectIOEvent += new _IOPOSScannerEvents_DirectIOEventEventHandler(_cco_DirectIOEvent);
                    _cco.ErrorEvent += new _IOPOSScannerEvents_ErrorEventEventHandler(_cco_ErrorEvent);
                    _cco.StatusUpdateEvent += new _IOPOSScannerEvents_StatusUpdateEventEventHandler(_cco_StatusUpdateEvent);
                }
                catch
                {
        ...
        #region OPOSScanner  Specific Properties
        ...
        public override byte[] ScanData
        {
            get
            {
                return InteropCommon.ToByteArrayFromString(_cco.ScanData, _binaryConversion);
            }
        }
        ...
        public override byte[] ScanDataLabel
        {
            get
            {
                return InteropCommon.ToByteArrayFromString(_cco.ScanDataLabel, _binaryConversion);
            }
        }

        public override BarCodeSymbology ScanDataType
        {
            get { return (BarCodeSymbology)InteropEnum<BarCodeSymbology>.ToEnumFromInteger(_cco.ScanDataType); }
        }

        #endregion OPOSScanner  Specific Properties
    }
}