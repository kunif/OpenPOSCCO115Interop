...
namespace OpenPOS.CCO115Interop
{
    ...
    [ServiceObject(DeviceType.Keylock, "OpenPOS 1.15 Keylock", "OPOS Keylock CCO Interop", 1, 15)]
    public class OpenPOSKeylock : Keylock, ILegacyControlObject, IDisposable
    {
        private OpenPOS.Devices.OPOSKeylock _cco = null;
        private const string _oposDeviceClass = "Keylock";
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (!_disposedValue)
            {
                ...
                if (_cco != null)
                {
                    _cco.DirectIOEvent -= (_IOPOSKeylockEvents_DirectIOEventEventHandler)_cco_DirectIOEvent;
                    _cco.StatusUpdateEvent -= (_IOPOSKeylockEvents_StatusUpdateEventEventHandler)_cco_StatusUpdateEvent;
                    _cco = null;
                }
        ...
        public override void Open()
        {
            ...
            if (_cco == null)
            {
                try
                {
                    ...
                    _cco = new OpenPOS.Devices.OPOSKeylock();

                    // Register event handler
                    _cco.DirectIOEvent += new _IOPOSKeylockEvents_DirectIOEventEventHandler(_cco_DirectIOEvent);
                    _cco.StatusUpdateEvent += new _IOPOSKeylockEvents_StatusUpdateEventEventHandler(_cco_StatusUpdateEvent);
                }
                catch
                {
        ...
        #region OPOSKeylock  Specific Properties

        public override KeylockType CapKeylockType
        {
            get { return (KeylockType)InteropEnum<KeylockType>.ToEnumFromInteger(_cco.CapKeylockType); }
        }

        public override byte[] ElectronicKeyValue
        {
            get { return InteropCommon.ToByteArrayFromString(_cco.ElectronicKeyValue, _binaryConversion); }
        }

        public override int KeyPosition
        {
            get { return _cco.KeyPosition; }
        }

        public override int PositionCount
        {
            get { return _cco.PositionCount; }
        }

        #endregion OPOSKeylock  Specific Properties

        #region OPOSKeylock  Specific Methodss

        public override void WaitForKeylockChange(int keyPosition, int timeout)
        {
            VerifyResult(_cco.WaitForKeylockChange(keyPosition, timeout));
        }

        #endregion OPOSKeylock  Specific Methodss
    }
}