...
namespace OpenPOS.CCO115Interop
{
    ...
    [ServiceObject(DeviceType.ItemDispenser, "OpenPOS 1.15 ItemDispenser", "OPOS ItemDispenser CCO Interop", 1, 15)]
    public class OpenPOSItemDispenser : ItemDispenser, ILegacyControlObject, IDisposable
    {
        private OpenPOS.Devices.OPOSItemDispenser _cco = null;
        private const string _oposDeviceClass = "ItemDispenser";
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (!_disposedValue)
            {
                ...
                if (_cco != null)
                {
                    _cco.DirectIOEvent -= (_IOPOSItemDispenserEvents_DirectIOEventEventHandler)_cco_DirectIOEvent;
                    _cco.StatusUpdateEvent -= (_IOPOSItemDispenserEvents_StatusUpdateEventEventHandler)_cco_StatusUpdateEvent;
                    _cco = null;
                }
        ...
        public override void Open()
        {
            ...
            if (_cco == null)
            {
                try
                {
                    ...
                    _cco = new OpenPOS.Devices.OPOSItemDispenser();

                    // Register event handler
                    _cco.DirectIOEvent += new _IOPOSItemDispenserEvents_DirectIOEventEventHandler(_cco_DirectIOEvent);
                    _cco.StatusUpdateEvent += new _IOPOSItemDispenserEvents_StatusUpdateEventEventHandler(_cco_StatusUpdateEvent);
                }
                catch
                {
        ...
        #region OPOSItemDispenser  Specific Properties
        ...
        public override bool CapIndividualSlotStatus
        {
            get { return _cco.CapIndividualSlotStatus; }
        }
        ...
        public override ItemDispenserStatus DispenserStatus
        {
            get { return (ItemDispenserStatus)InteropEnum<ItemDispenserStatus>.ToEnumFromInteger(_cco.DispenserStatus); }
        }

        public override int MaxSlots
        {
            get { return _cco.MaxSlots; }
        }

        #endregion OPOSItemDispenser  Specific Properties

        #region OPOSItemDispenser  Specific Methodss

        public override void AdjustItemCount(int itemCount, int slotNumber)
        {
            VerifyResult(_cco.AdjustItemCount(itemCount, slotNumber));
        }

        public override void DispenseItem(int numItem, int slotNumber)
        {
            VerifyResult(_cco.DispenseItem(numItem, slotNumber));
        }

        public override int ReadItemCount(int slotNumber)
        {
            int itemCount = 0;
            VerifyResult(_cco.ReadItemCount(ref itemCount, slotNumber));
            return itemCount;
        }

        #endregion OPOSItemDispenser  Specific Methodss
    }
}