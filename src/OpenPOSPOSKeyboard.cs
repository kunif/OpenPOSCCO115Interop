...
namespace OpenPOS.CCO115Interop
{
    ...
    [ServiceObject(DeviceType.PosKeyboard, "OpenPOS 1.15 POSKeyboard", "OPOS POSKeyboard CCO Interop", 1, 15)]
    public class OpenPOSPOSKeyboard : PosKeyboard, ILegacyControlObject, IDisposable
    {
        private OpenPOS.Devices.OPOSPOSKeyboard _cco = null;
        private const string _oposDeviceClass = "POSKeyboard";
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (!_disposedValue)
            {
                ...
                if (_cco != null)
                {
                    _cco.DataEvent -= (_IOPOSPOSKeyboardEvents_DataEventEventHandler)_cco_DataEvent;
                    _cco.DirectIOEvent -= (_IOPOSPOSKeyboardEvents_DirectIOEventEventHandler)_cco_DirectIOEvent;
                    _cco.ErrorEvent -= (_IOPOSPOSKeyboardEvents_ErrorEventEventHandler)_cco_ErrorEvent;
                    _cco.StatusUpdateEvent -= (_IOPOSPOSKeyboardEvents_StatusUpdateEventEventHandler)_cco_StatusUpdateEvent;
                    _cco = null;
                }
        ...
        public override void Open()
        {
            ...
            if (_cco == null)
            {
                try
                {
                    ...
                    _cco = new OpenPOS.Devices.OPOSPOSKeyboard();

                    // Register event handler
                    _cco.DataEvent += new _IOPOSPOSKeyboardEvents_DataEventEventHandler(_cco_DataEvent);
                    _cco.DirectIOEvent += new _IOPOSPOSKeyboardEvents_DirectIOEventEventHandler(_cco_DirectIOEvent);
                    _cco.ErrorEvent += new _IOPOSPOSKeyboardEvents_ErrorEventEventHandler(_cco_ErrorEvent);
                    _cco.StatusUpdateEvent += new _IOPOSPOSKeyboardEvents_StatusUpdateEventEventHandler(_cco_StatusUpdateEvent);
                }
                catch
                {
        ...
        #region OPOSPOSKeyboard  Specific Properties

        public override bool CapKeyUp
        {
            get { return _cco.CapKeyUp; }
        }

        public override KeyboardEventType EventTypes
        {
            get
            {
                return (KeyboardEventType)InteropEnum<KeyboardEventType>.ToEnumFromInteger(_cco.EventTypes);
            }
            set
            {
                _cco.EventTypes = (int)value;
                VerifyResult(_cco.ResultCode);
            }
        }

        public override int PosKeyData
        {
            get { return _cco.POSKeyData; }
        }

        public override KeyEvent PosKeyEventType
        {
            get { return (KeyEvent)InteropEnum<KeyEvent>.ToEnumFromInteger(_cco.POSKeyEventType); }
        }

        #endregion OPOSPOSKeyboard  Specific Properties
    }
}