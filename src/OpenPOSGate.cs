...
namespace OpenPOS.CCO115Interop
{
    ...
    [ServiceObject(DeviceType.Gate, "OpenPOS 1.15 Gate", "OPOS Gate CCO Interop", 1, 15)]
    public class OpenPOSGate : Gate, ILegacyControlObject, IDisposable
    {
        private OpenPOS.Devices.OPOSGate _cco = null;
        private const string _oposDeviceClass = "Gate";
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (!_disposedValue)
            {
                ...
                if (_cco != null)
                {
                    _cco.DirectIOEvent -= (_IOPOSGateEvents_DirectIOEventEventHandler)_cco_DirectIOEvent;
                    _cco.StatusUpdateEvent -= (_IOPOSGateEvents_StatusUpdateEventEventHandler)_cco_StatusUpdateEvent;
                    _cco = null;
                }
        ...
        public override void Open()
        {
            ...
            if (_cco == null)
            {
                try
                {
                    ...
                    _cco = new OpenPOS.Devices.OPOSGate();

                    // Register event handler
                    _cco.DirectIOEvent += new _IOPOSGateEvents_DirectIOEventEventHandler(_cco_DirectIOEvent);
                    _cco.StatusUpdateEvent += new _IOPOSGateEvents_StatusUpdateEventEventHandler(_cco_StatusUpdateEvent);
                }
                catch
                {
        ...
        #region OPOSGate  Specific Properties

        public override bool CapGateStatus
        {
            get { return _cco.CapGateStatus; }
        }

        public override GateStatus GateStatus
        {
            get { return (GateStatus)InteropEnum<GateStatus>.ToEnumFromInteger(_cco.GateStatus); }
        }

        #endregion OPOSGate  Specific Properties

        #region OPOSGate  Specific Methodss

        public override void OpenGate()
        {
            VerifyResult(_cco.OpenGate());
        }

        public override void WaitForGateClose(int timeout)
        {
            VerifyResult(_cco.WaitForGateClose(timeout));
        }

        #endregion OPOSGate  Specific Methodss
    }
}