...
namespace OpenPOS.CCO115Interop
{
    ...
    [ServiceObject(DeviceType.CashDrawer, "OpenPOS 1.15 CashDrawer", "OPOS CashDrawer CCO Interop", 1, 15)]
    public class OpenPOSCashDrawer : CashDrawer, ILegacyControlObject, IDisposable
    {
        private OpenPOS.Devices.OPOSCashDrawer _cco = null;
        private const string _oposDeviceClass = "CashDrawer";
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (!_disposedValue)
            {
                ...
                if (_cco != null)
                {
                    _cco.DirectIOEvent -= (_IOPOSCashDrawerEvents_DirectIOEventEventHandler)_cco_DirectIOEvent;
                    _cco.StatusUpdateEvent -= (_IOPOSCashDrawerEvents_StatusUpdateEventEventHandler)_cco_StatusUpdateEvent;
                    _cco = null;
                }
        ...
        public override void Open()
        {
            ...
            if (_cco == null)
            {
                try
                {
                    ...
                    _cco = new OpenPOS.Devices.OPOSCashDrawer();

                    // Register event handler
                    _cco.DirectIOEvent += new _IOPOSCashDrawerEvents_DirectIOEventEventHandler(_cco_DirectIOEvent);
                    _cco.StatusUpdateEvent += new _IOPOSCashDrawerEvents_StatusUpdateEventEventHandler(_cco_StatusUpdateEvent);
                }
                catch
                {
        ...
        #region OPOSCashDrawer  Specific Properties

        public override bool CapStatus
        {
            get { return _cco.CapStatus; }
        }

        public override bool CapStatusMultiDrawerDetect
        {
            get { return _cco.CapStatusMultiDrawerDetect; }
        }

        public override bool DrawerOpened
        {
            get { return _cco.DrawerOpened; }
        }

        #endregion OPOSCashDrawer  Specific Properties

        #region OPOSCashDrawer  Specific Methodss

        public override void OpenDrawer()
        {
            VerifyResult(_cco.OpenDrawer());
        }

        public override void WaitForDrawerClose(int beepTimeout, int beepFrequency, int beepDuration, int beepDelay)
        {
            VerifyResult(_cco.WaitForDrawerClose(beepTimeout, beepFrequency, beepDuration, beepDelay));
        }

        #endregion OPOSCashDrawer  Specific Methodss
    }
}