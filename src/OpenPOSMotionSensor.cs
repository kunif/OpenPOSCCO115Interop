...
namespace OpenPOS.CCO115Interop
{
    ...
    [ServiceObject(DeviceType.MotionSensor, "OpenPOS 1.15 MotionSensor", "OPOS MotionSensor CCO Interop", 1, 15)]
    public class OpenPOSMotionSensor : MotionSensor, ILegacyControlObject, IDisposable
    {
        private OpenPOS.Devices.OPOSMotionSensor _cco = null;
        private const string _oposDeviceClass = "MotionSensor";
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (!_disposedValue)
            {
                ...
                if (_cco != null)
                {
                    _cco.DirectIOEvent -= (_IOPOSMotionSensorEvents_DirectIOEventEventHandler)_cco_DirectIOEvent;
                    _cco.StatusUpdateEvent -= (_IOPOSMotionSensorEvents_StatusUpdateEventEventHandler)_cco_StatusUpdateEvent;
                    _cco = null;
                }
        ...
        public override void Open()
        {
            ...
            if (_cco == null)
            {
                try
                {
                    ...
                    _cco = new OpenPOS.Devices.OPOSMotionSensor();

                    // Register event handler
                    _cco.DirectIOEvent += new _IOPOSMotionSensorEvents_DirectIOEventEventHandler(_cco_DirectIOEvent);
                    _cco.StatusUpdateEvent += new _IOPOSMotionSensorEvents_StatusUpdateEventEventHandler(_cco_StatusUpdateEvent);
                }
                catch
                {
        ...
        #region OPOSMotionSensor  Specific Properties

        public override bool Motion
        {
            get { return _cco.Motion; }
        }
        ...
        #endregion OPOSMotionSensor  Specific Properties

        #region OPOSMotionSensor  Specific Methodss

        public override void WaitForMotion(int timeout)
        {
            VerifyResult(_cco.WaitForMotion(timeout));
        }

        #endregion OPOSMotionSensor  Specific Methodss
    }
}