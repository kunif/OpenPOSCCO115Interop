...
namespace OpenPOS.CCO115Interop
{
    ...
    [ServiceObject(DeviceType.SignatureCapture, "OpenPOS 1.15 SignatureCapture", "OPOS SignatureCapture CCO Interop", 1, 15)]
    public class OpenPOSSignatureCapture : SignatureCapture, ILegacyControlObject, IDisposable
    {
        private OpenPOS.Devices.OPOSSigCap _cco = null;
        private const string _oposDeviceClass = "SignatureCapture";
        ...
        public OpenPOSSignatureCapture()
        {
            _cco = null;
            _oposDeviceName = "";
            _binaryConversion = 0;
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (!_disposedValue)
            {
                ...
                if (_cco != null)
                {
                    _cco.DataEvent -= (_IOPOSSigCapEvents_DataEventEventHandler)_cco_DataEvent;
                    _cco.DirectIOEvent -= (_IOPOSSigCapEvents_DirectIOEventEventHandler)_cco_DirectIOEvent;
                    _cco.ErrorEvent -= (_IOPOSSigCapEvents_ErrorEventEventHandler)_cco_ErrorEvent;
                    _cco.StatusUpdateEvent -= (_IOPOSSigCapEvents_StatusUpdateEventEventHandler)_cco_StatusUpdateEvent;
                    _cco = null;
                }
        ...
        public override void Open()
        {
            ...
            if (_cco == null)
            {
                try
                {
                    ...
                    _cco = new OpenPOS.Devices.OPOSSigCap();

                    // Register event handler
                    _cco.DataEvent += new _IOPOSSigCapEvents_DataEventEventHandler(_cco_DataEvent);
                    _cco.DirectIOEvent += new _IOPOSSigCapEvents_DirectIOEventEventHandler(_cco_DirectIOEvent);
                    _cco.ErrorEvent += new _IOPOSSigCapEvents_ErrorEventEventHandler(_cco_ErrorEvent);
                    _cco.StatusUpdateEvent += new _IOPOSSigCapEvents_StatusUpdateEventEventHandler(_cco_StatusUpdateEvent);
                }
                catch
                {
        ...
        #region OPOSSignatureCapture  Specific Properties
        ...
        public override bool CapUserTerminated
        {
            get { return _cco.CapDisplay; }
        }
        ...
        public override System.Drawing.Point[] PointArray
        {
            get
            {
                byte[] array = InteropCommon.ToByteArrayFromString(_cco.PointArray, _binaryConversion);

                if (array != null && array.Length >= 4)
                {
                    int num = array.Length / 4;
                    Point[] array2 = new Point[num];

                    for (int i = 0; i < num; i++)
                    {
                        int num2 = i * 4;
                        int x = array[num2] + (array[num2 + 1] << 8);
                        int y = array[num2 + 2] + (array[num2 + 3] << 8);
        ...
        {
            get { return InteropCommon.ToByteArrayFromString(_cco.RawData, _binaryConversion); }
        }
        ...
        #endregion OPOSSignatureCapture  Specific Properties

        #region OPOSSignatureCapture  Specific Methodss

        public override void BeginCapture(string formName)
        {
            VerifyResult(_cco.BeginCapture(formName));
        }
        ...
        #endregion OPOSSignatureCapture  Specific Methodss
    }
}